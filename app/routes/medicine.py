...
def classify_risk(condition, probability):
    ...
    if probability <= 60:
        ...
        if condition == "당뇨":
            ...
            message = "지금 체질량지수(BMI)와 건강 상태로 본 복부비만 위험이 낮은 상태예요, 지금처럼 건강한 식사와 운동을 꾸준히 실천하시면서, 활기차고 건강한 생활을 이어가봐요."
        ...
        if condition == "당뇨":
            ...
            message = "혈압 수치가 높아 심혈관 질환 같은 합병증 위험이 커질 수 있어요. 가공식품이나 인스턴트 음식은 꼭 피하시 게 좋아요. 무엇보다 전문가의 도움을 받는 게 가장 중요해요. 병원을 방문하셔서 의사와 상담을 통해 필요한 약물 치료나 검사를 받아보세요."
        elif condition == "복부비만":
            message = "현재 복부비만 위험이 높은 단계에 속해 있어요. 이 상태에서는 당뇨, 고혈압, 심혈관 질환 같은 건강 문제로 이어질 수 있어요. 과도한 칼로리를 줄이고, 섬유질이 풍부한 식사를 하며 유산소 운동도 꾸준히 실천하시면 좋아요."
        elif condition == "이상지질혈증":
